<?php
	...
	/*
	* Fetching all the connections of a particular symptom (This is used in backup section)  
	*/
?>
<?php  
	$resultData = array();
	...
	try {
		if(isset($_POST['initial_symptom_id']) AND $_POST['initial_symptom_id'] != ""){	
			...
			* Connect section
			*/
			$connectedSymptomResult = mysqli_query($db,"SELECT id, saved_comparisons_backup_id, source_arznei_id, initial_source_type, comparing_source_type, is_saved, initial_source_id, comparing_source_id, conversion_initial_source_id, conversion_comparing_source_id, initial_source_code, comparing_source_code, initial_source_symptom_id, comparing_source_symptom_id, initial_source_symptom_highlighted_de, initial_source_symptom_highlighted_en, comparing_source_symptom_highlighted_de, comparing_source_symptom_highlighted_en, initial_source_symptom_de, initial_source_symptom_en, comparing_source_symptom_de, comparing_source_symptom_en, connection_language, matching_percentage, is_connected, is_ns_connect, ns_connect_note, is_pasted, is_ns_paste, ns_paste_note, connection_or_paste_type FROM symptom_connections_backup WHERE ((initial_source_symptom_id = '".$workingSymptomId."' AND initial_source_type = 'backup') OR (comparing_source_symptom_id = '".$workingSymptomId."' AND comparing_source_type = 'backup')) AND is_connected = 1 ORDER BY matching_percentage DESC");
			if(mysqli_num_rows($connectedSymptomResult) > 0){
				while($connectedRow = mysqli_fetch_array($connectedSymptomResult)) {
					...
					if($workingSymptomId == $connectedRow['initial_source_symptom_id']){
						...
						if($connectedRow['comparing_source_type'] == "backup")
							$quelle_import_test_table = 'quelle_import_backup';
						else
							$quelle_import_test_table = 'quelle_import_test';
					}
					else{
						...
						if($connectedRow['initial_source_type'] == "backup")
							$quelle_import_test_table = 'quelle_import_backup';
						else
							$quelle_import_test_table = 'quelle_import_test';
					}
					...
					$symptomInfoResult = mysqli_query($db, "SELECT Kommentar, Fussnote, is_final_version_available FROM ".$quelle_import_test_table." WHERE id = '".$innerWorkingSymptomId."'");
					...
					if($connectedRow['initial_source_id'] != ""){
						if($connectedRow['initial_source_type'] == "backup")
							...
							$quelle_table = 'quelle';
						$InitialSymptomResult = mysqli_query($db,"SELECT code, jahr, quelle_type_id FROM ".$quelle_table." WHERE quelle_id = '".$connectedRow['initial_source_id']."'");
					...
					if($connectedRow['comparing_source_id'] != ""){
						if($connectedRow['comparing_source_type'] == "backup")
							...
							$quelle_table = 'quelle';
						$comparingSymptomResult = mysqli_query($db,"SELECT code, jahr, quelle_type_id FROM ".$quelle_table." WHERE quelle_id = '".$connectedRow['comparing_source_id']."'");
					...
					$initialArzId = "";
					if($connectedRow['initial_source_type'] == "backup"){
						$originInitialQuelleResult = mysqli_query($db,"SELECT quelle.jahr, quelle.quelle_type_id, quelle_import_backup.original_symptom_id FROM quelle_import_backup LEFT JOIN quelle ON quelle_import_backup.original_quelle_id = quelle.quelle_id WHERE quelle_import_backup.id = '".$connectedRow['initial_source_symptom_id']."'");
						if(mysqli_num_rows($originInitialQuelleResult) > 0){
							...
							// fetch original symptom info
							if(isset($originIniQuelleRow['original_symptom_id']) AND $originIniQuelleRow['original_symptom_id'] != ""){
								$originalSymptomInfoResult = mysqli_query($db,"SELECT original_quelle_id, arznei_id FROM quelle_import_test WHERE id = '".$originIniQuelleRow['original_symptom_id']."'");
					...
					{
						$originInitialQuelleResult = mysqli_query($db,"SELECT quelle.jahr, quelle.quelle_type_id, quelle_import_test.original_quelle_id, quelle_import_test.arznei_id, quelle_import_test.id FROM quelle_import_test LEFT JOIN quelle ON quelle_import_test.original_quelle_id = quelle.quelle_id WHERE quelle_import_test.id = '".$connectedRow['initial_source_symptom_id']."'");
						if(mysqli_num_rows($originInitialQuelleResult) > 0){
							$originIniQuelleRow = mysqli_fetch_assoc($originInitialQuelleResult);
							$originInitialSourceYear = $originIniQuelleRow['jahr'];
							...
							// $backupSetSwappedSymptomResult = mysqli_query($db,"SELECT original_quelle_id, arznei_id FROM backup_sets_swapped_symptoms WHERE original_symptom_id = '".$originIniQuelleRow['id']."' AND saved_comparisons_backup_id = '".$connectedRow['saved_comparisons_backup_id']."'");
							// if(mysqli_num_rows($backupSetSwappedSymptomResult) > 0){
							// 	$backupSetSymptomRow = mysqli_fetch_assoc($backupSetSwappedSymptomResult);
							// 	$initialOrzId = $backupSetSymptomRow['original_quelle_id'];
							// 	$initialArzId = $backupSetSymptomRow['arznei_id'];
							// }
					...
					$originInitialQuelleResult = mysqli_query($db,"SELECT quelle.jahr, quelle.quelle_type_id, quelle_import_backup.id, quelle_import_backup.original_symptom_id, quelle_import_backup.is_final_version_available FROM quelle_import_backup LEFT JOIN quelle ON quelle_import_backup.original_quelle_id = quelle.quelle_id WHERE quelle_import_backup.id = '".$connectedRow['initial_source_symptom_id']."'");
					if(mysqli_num_rows($originInitialQuelleResult) > 0){
						$originIniQuelleRow = mysqli_fetch_assoc($originInitialQuelleResult);
						$originInitialSourceYear = $originIniQuelleRow['jahr'];
					...
					$comparingArzId = "";
					if($connectedRow['comparing_source_type'] == "backup"){
						$originComparingQuelleResult = mysqli_query($db,"SELECT quelle.jahr, quelle.quelle_type_id, quelle_import_backup.original_symptom_id FROM quelle_import_backup LEFT JOIN quelle ON quelle_import_backup.original_quelle_id = quelle.quelle_id WHERE quelle_import_backup.id = '".$connectedRow['comparing_source_symptom_id']."'");
						if(mysqli_num_rows($originComparingQuelleResult) > 0){
							...
							// fetch original symptom info
							if(isset($originComQuelleRow['original_symptom_id']) AND $originComQuelleRow['original_symptom_id'] != ""){
								$originalSymptomInfoResult = mysqli_query($db,"SELECT original_quelle_id, arznei_id FROM quelle_import_test WHERE id = '".$originComQuelleRow['original_symptom_id']."'");
								if(mysqli_num_rows($originalSymptomInfoResult) > 0)
									$originalSymptomInfoData = mysqli_fetch_assoc($originalSymptomInfoResult);
							}
							$comparingOrzId = (isset($originalSymptomInfoData['original_quelle_id']) AND $originalSymptomInfoData['original_quelle_id'] != "") ? $originalSymptomInfoData['original_quelle_id'] : "";
							$comparingArzId = (isset($originalSymptomInfoData['arznei_id']) AND $originalSymptomInfoData['arznei_id'] != "") ? $originalSymptomInfoData['arznei_id'] : "";
						}
					}
					else
					{
						...
						if(mysqli_num_rows($originComparingQuelleResult) > 0){
							...
							// $backupSetSwappedSymptomResult = mysqli_query($db,"SELECT original_quelle_id, arznei_id FROM backup_sets_swapped_symptoms WHERE original_symptom_id = '".$originComQuelleRow['id']."' AND saved_comparisons_backup_id = '".$connectedRow['saved_comparisons_backup_id']."'");
							// if(mysqli_num_rows($backupSetSwappedSymptomResult) > 0){
							// 	$backupSetSymptomRow = mysqli_fetch_assoc($backupSetSwappedSymptomResult);
							// 	$comparingOrzId = $backupSetSymptomRow['original_quelle_id'];
							// 	$comparingArzId = $backupSetSymptomRow['arznei_id'];
							// }
						}
					}*/
					...
					$originComparingQuelleResult = mysqli_query($db,"SELECT quelle.jahr, quelle.quelle_type_id, quelle_import_backup.id, quelle_import_backup.original_symptom_id, quelle_import_backup.is_final_version_available FROM quelle_import_backup LEFT JOIN quelle ON quelle_import_backup.original_quelle_id = quelle.quelle_id WHERE quelle_import_backup.id = '".$connectedRow['comparing_source_symptom_id']."'");
					if(mysqli_num_rows($originComparingQuelleResult) > 0){
						$originComQuelleRow = mysqli_fetch_assoc($originComparingQuelleResult);
						$originComparingSourceYear = $originComQuelleRow['jahr'];
					...
					$data['saved_comparisons_backup_id'] = $connectedRow['saved_comparisons_backup_id'];
					...
					$data['initial_source_type'] = $connectedRow['initial_source_type'];
					$data['comparing_source_type'] = $connectedRow['comparing_source_type'];
			...
			*/
			...
			$pastedSymptomResult = mysqli_query($db,"SELECT id, saved_comparisons_backup_id, source_arznei_id, initial_source_type, comparing_source_type, is_saved, initial_source_id, comparing_source_id, conversion_initial_source_id, conversion_comparing_source_id, initial_source_code, comparing_source_code, initial_source_symptom_id, comparing_source_symptom_id, initial_source_symptom_highlighted_de, initial_source_symptom_highlighted_en, comparing_source_symptom_highlighted_de, comparing_source_symptom_highlighted_en, initial_source_symptom_de, initial_source_symptom_en, comparing_source_symptom_de, comparing_source_symptom_en, connection_language, matching_percentage, is_connected, is_ns_connect, ns_connect_note, is_pasted, is_ns_paste, ns_paste_note, connection_or_paste_type FROM symptom_connections_backup WHERE ((initial_source_symptom_id = '".$workingSymptomId."' AND initial_source_type = 'backup') OR (comparing_source_symptom_id = '".$workingSymptomId."' AND comparing_source_type = 'backup')) AND is_pasted = 1 ORDER BY matching_percentage DESC");
			if(mysqli_num_rows($pastedSymptomResult) > 0){
				while($pastedRow = mysqli_fetch_array($pastedSymptomResult)) {
					...
					if($workingSymptomId == $pastedRow['initial_source_symptom_id']){
						...
						if($pastedRow['comparing_source_type'] == "backup")
							$quelle_import_test_table = 'quelle_import_backup';
						else
							$quelle_import_test_table = 'quelle_import_test';
					}
						...
						if($pastedRow['initial_source_type'] == "backup")
							$quelle_import_test_table = 'quelle_import_backup';
						else
							$quelle_import_test_table = 'quelle_import_test';
					}
					...
					if($pastedRow['initial_source_id'] != ""){
						if($pastedRow['initial_source_type'] == "backup")
							...
							$quelle_table = 'quelle';
						$InitialSymptomResult = mysqli_query($db,"SELECT code, jahr, quelle_type_id FROM ".$quelle_table." WHERE quelle_id = '".$pastedRow['initial_source_id']."'");
						if(mysqli_num_rows($InitialSymptomResult) > 0){
							$iniSymRow = mysqli_fetch_assoc($InitialSymptomResult);
							if($iniSymRow['quelle_type_id'] == 3)
								$preparedQuelleCode = $iniSymRow['code'];
					...
					if($pastedRow['comparing_source_id'] != ""){
						if($pastedRow['comparing_source_type'] == "backup")
							...
							$quelle_table = 'quelle';
						$comparingSymptomResult = mysqli_query($db,"SELECT code, jahr, quelle_type_id FROM ".$quelle_table." WHERE quelle_id = '".$pastedRow['comparing_source_id']."'");
						if(mysqli_num_rows($comparingSymptomResult) > 0){
							$comparingSymptomRow = mysqli_fetch_assoc($comparingSymptomResult);
							if($comparingSymptomRow['quelle_type_id'] == 3)
								$preparedQuelleCodeForCom = $comparingSymptomRow['code'];
					...
					$initialArzId = "";
					if($pastedRow['initial_source_type'] == "backup"){
						$originInitialQuelleResult = mysqli_query($db,"SELECT quelle.jahr, quelle.quelle_type_id, quelle_import_backup.original_symptom_id FROM quelle_import_backup LEFT JOIN quelle ON quelle_import_backup.original_quelle_id = quelle.quelle_id WHERE quelle_import_backup.id = '".$pastedRow['initial_source_symptom_id']."'");
						if(mysqli_num_rows($originInitialQuelleResult) > 0){
							$originIniQuelleRow = mysqli_fetch_assoc($originInitialQuelleResult);
							$originInitialSourceYear = $originIniQuelleRow['jahr'];
							// fetch original symptom info
					...
					else
					{
						...
						if(mysqli_num_rows($originInitialQuelleResult) > 0){
							...
							$backupSetSwappedSymptomResult = mysqli_query($db,"SELECT original_quelle_id, arznei_id FROM backup_sets_swapped_symptoms WHERE original_symptom_id = '".$originIniQuelleRow['id']."' AND saved_comparisons_backup_id = '".$pastedRow['saved_comparisons_backup_id']."'");
							if(mysqli_num_rows($backupSetSwappedSymptomResult) > 0){
								$backupSetSymptomRow = mysqli_fetch_assoc($backupSetSwappedSymptomResult);
								$initialOrzId = $backupSetSymptomRow['original_quelle_id'];
								$initialArzId = $backupSetSymptomRow['arznei_id'];
					...
					$originInitialSourceYear = "";
					$iniIsFinalVersionAvailable = 0;
					$originInitialQuelleResult = mysqli_query($db,"SELECT quelle.jahr, quelle.quelle_type_id, quelle_import_backup.id, quelle_import_backup.original_symptom_id, quelle_import_backup.is_final_version_available FROM quelle_import_backup LEFT JOIN quelle ON quelle_import_backup.original_quelle_id = quelle.quelle_id WHERE quelle_import_backup.id = '".$pastedRow['initial_source_symptom_id']."'");
					if(mysqli_num_rows($originInitialQuelleResult) > 0){
						$originIniQuelleRow = mysqli_fetch_assoc($originInitialQuelleResult);
						$originInitialSourceYear = $originIniQuelleRow['jahr'];
						$iniIsFinalVersionAvailable = $originIniQuelleRow['is_final_version_available'];
					...
					$comparingArzId = "";
					if($pastedRow['comparing_source_type'] == "backup"){
						$originComparingQuelleResult = mysqli_query($db,"SELECT quelle.jahr, quelle.quelle_type_id, quelle_import_backup.original_symptom_id FROM quelle_import_backup LEFT JOIN quelle ON quelle_import_backup.original_quelle_id = quelle.quelle_id WHERE quelle_import_backup.id = '".$pastedRow['comparing_source_symptom_id']."'");
						if(mysqli_num_rows($originComparingQuelleResult) > 0){
							$originComQuelleRow = mysqli_fetch_assoc($originComparingQuelleResult);
							$originComparingSourceYear = $originComQuelleRow['jahr'];
							// fetch original symptom info
					...
					else
					{
						...
						if(mysqli_num_rows($originComparingQuelleResult) > 0){
							...
							$backupSetSwappedSymptomResult = mysqli_query($db,"SELECT original_quelle_id, arznei_id FROM backup_sets_swapped_symptoms WHERE original_symptom_id = '".$originComQuelleRow['id']."' AND saved_comparisons_backup_id = '".$pastedRow['saved_comparisons_backup_id']."'");
							if(mysqli_num_rows($backupSetSwappedSymptomResult) > 0){
								$backupSetSymptomRow = mysqli_fetch_assoc($backupSetSwappedSymptomResult);
								$comparingOrzId = $backupSetSymptomRow['original_quelle_id'];
								$comparingArzId = $backupSetSymptomRow['arznei_id'];
					...
					$originComparingSourceYear = "";
					$comIsFinalVersionAvailable = 0;
					$originComparingQuelleResult = mysqli_query($db,"SELECT quelle.jahr, quelle.quelle_type_id, quelle_import_backup.id, quelle_import_backup.original_symptom_id, quelle_import_backup.is_final_version_available FROM quelle_import_backup LEFT JOIN quelle ON quelle_import_backup.original_quelle_id = quelle.quelle_id WHERE quelle_import_backup.id = '".$pastedRow['comparing_source_symptom_id']."'");
					if(mysqli_num_rows($originComparingQuelleResult) > 0){
						$originComQuelleRow = mysqli_fetch_assoc($originComparingQuelleResult);
						$originComparingSourceYear = $originComQuelleRow['jahr'];
						$comIsFinalVersionAvailable = $originComQuelleRow['is_final_version_available'];
					...
					$data['saved_comparisons_backup_id'] = $pastedRow['saved_comparisons_backup_id'];
					...
					$data['comparing_source_original_language'] = $comparingOrigianlSourceOriginalLanguage; // Comparing original source original language
					$data['initial_source_type'] = $pastedRow['initial_source_type'];
					$data['comparing_source_type'] = $pastedRow['comparing_source_type'];
