...
switch($symptomType){
			...
			foreach ($singleConnectionInitialCheckArray as $key => $value) {
				...
				if(mysqli_num_rows($symptomConnectionCollectionQuery) > 0){
					while($row = mysqli_fetch_array($symptomConnectionCollectionQuery)){
						...
						//For chapter assignment normal display
						$comparing_symptom_de_raw = $comparing_symptom_de;
						$comparing_symptom_en_raw = $comparing_symptom_en;
						...
						$data = array(
							...
							'comparing_symptom_de_raw'=> $comparing_symptom_de_raw,
							'comparing_symptom_en_raw'=> $comparing_symptom_en_raw
	...
	case "combined-initial":{
		if($combinedConnectionIntialsCheckArray != ""){
			...
			if(!empty($arrayToSend)){
				usort($arrayToSend, 'recentConnectSort');
			}
		}
		break;
...
function recentConnectSort($a, $b) {
   return  $b['free_flag'] - $a['free_flag'];
}
