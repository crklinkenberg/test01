<?php
	...
	/*
	* Displaying a saved comparison's all symptoms and their connections
	*/
?>
<?php  
	...
	$qid = "";
	if($mid != ""){
		$scResult = mysqli_query($db,"SELECT Q.quelle_id, Q.is_materia_medica FROM quelle AS Q LEFT JOIN quelle_import_master AS QM ON Q.quelle_id = QM.quelle_id WHERE QM.id = '".$mid."'");
...
<head>
	<meta charset="utf-8">
	<title>Source's connections</title>
...
<body>
					...
					<form id="batch_result_form_1" class="batch-result-form append-recognizer">
						<table class="table table-bordered">
							<?php
								if($mid != "")
								{
									$symResult = mysqli_query($db, "SELECT QI.id, QI.original_symptom_id, QI.quelle_code, QI.quelle_id, QI.original_quelle_id, QI.arznei_id, QI.final_version_de, QI.final_version_en, QI.BeschreibungPlain_de, QI.BeschreibungPlain_en, QI.BeschreibungOriginal_de, QI.BeschreibungOriginal_en, QI.searchable_text_de, QI.searchable_text_en, QI.is_final_version_available, QI.Kommentar, QI.Fussnote, QIM.arznei_id, SC.comparison_option, SC.comparison_language FROM quelle_import_test AS QI JOIN saved_comparisons as SC ON QI.quelle_id = SC.quelle_id JOIN quelle_import_master AS QIM ON QI.quelle_id = QIM.quelle_id WHERE QIM.id = '".$mid."' AND QI.master_id = '".$mid."'");
										...
										while($symRow = mysqli_fetch_array($symResult)){
											...
											if($symRow['is_final_version_available'] != 0){
												...
												if(isset($symRow['comparison_option']) AND $symRow['comparison_option'] == 1){
													...
													$iniSymptomString_de =  $symRow['BeschreibungOriginal_de'];
													$iniSymptomString_en =  $symRow['BeschreibungOriginal_en'];
												}
											}
											...
											// convertTheSymptom($iniSymptomString_de, $symRow['original_quelle_id'], $symRow['arznei_id'], $symRow['is_final_version_available'])
											...
											$ceheckConnectionResult = mysqli_query($db,"SELECT id, is_saved FROM symptom_connections WHERE (initial_source_symptom_id = '".$symRow['id']."' OR comparing_source_symptom_id = '".$symRow['id']."') AND (is_connected = 1 OR is_pasted = 1)");
											...
											$getQuelleResult = mysqli_query($db,"SELECT code, jahr, quelle_type_id FROM quelle WHERE quelle_id = '".$symRow['quelle_id']."'");
											...
											<tr id="row_<?php echo $symRow['id']; ?>" class="initial-source-symptom-row">
												...
												<th style="width: 15%;">
													<ul class="info-linkage-group">
														<li>
															<a onclick="showInfo(<?php echo $symRow['id']; ?>)" title="info" href="javascript:void(0)" data-item="info" data-unique-id="<?php echo $symRow['id']; ?>"><i class="fas fa-info-circle"></i></a>
														</li>
														<li>
															<a class="<?php echo $commentClasses; ?>" id="comment_icon_<?php echo $symRow['id']; ?>" onclick="showComment(<?php echo $symRow['id']; ?>, <?php echo $symRow['id']; ?>)" title="comment" href="javascript:void(0)" data-item="comment" data-unique-id="<?php echo $symRow['id']; ?>"><i class="fas fa-comment-alt"></i></a>
														</li>
														<li>
															<a class="<?php echo $footnoteClasses; ?>" id="footnote_icon_<?php echo $symRow['id']; ?>" onclick="showFootnote(<?php echo $symRow['id']; ?>, <?php echo $symRow['id']; ?>)" title="footnote" href="javascript:void(0)" data-item="footnote" data-unique-id="<?php echo $symRow['id']; ?>"><i class="fas fa-sticky-note"></i></a>
														</li>
														<li>
	...
	<script type="text/javascript">
		...
		$(document).on('click', '.vbtn-has-connection', function(){
		    ...
		    if(isConnectionLoaded == 1){
				...
				$("#v_btn_"+parentUniqueId).promise().done(function(){
					...
					$.ajax({
						type: 'POST',
						url: 'get-symptom-connections-view-only.php',
						data: {
								...
								$.each(resultData, function( key, value ) {
									...
									// }
							  		...
							  		if(value.is_initial_source == 1){
								  		...
								  		html += '			<li>';
								  		html += '				<a href="javascript:void(0)" id="v_btn_'+uniqueId+'" class="'+vBtnClasses+' '+vBtnDisable+'" title="'+vBtnTitle+'" data-unique-id="'+uniqueId+'" data-v-padding="'+setVpadding+'" data-initial-symptom-id="'+value.initial_source_symptom_id+'"  data-comparing-symptom-id="'+value.comparing_source_symptom_id+'" data-active-symptom-type="initial" data-main-parent-initial-symptom-id="'+mainParentInitialSymptomId+'" data-is-connection-loaded="0" data-source-arznei-id="'+sourceArzneiId+'" data-removable-row-class-chain="'+removableRowClassChain+'"><i class="fas fa-plus"></i></a>';
								  		html += '			</li>';
								  		html += '		</ul>';
							  		}
							  		else
							  		{
								  		...
								  		html += '			</li>';
								  		html += '			<li>';
								  		html += '				<a href="javascript:void(0)" id="v_btn_'+uniqueId+'" class="'+vBtnClasses+' '+vBtnDisable+'" title="'+vBtnTitle+'" data-unique-id="'+uniqueId+'" data-v-padding="'+setVpadding+'" data-initial-symptom-id="'+value.initial_source_symptom_id+'" data-comparing-symptom-id="'+value.comparing_source_symptom_id+'" data-active-symptom-type="comparing" data-main-parent-initial-symptom-id="'+mainParentInitialSymptomId+'" data-is-connection-loaded="0" data-source-arznei-id="'+sourceArzneiId+'" data-removable-row-class-chain="'+removableRowClassChain+'"><i class="fas fa-plus"></i></a>';
								  		html += '			</li>';
								  		html += '		</ul>';
							  		}
		...
		/*
		// THIS FUNCTION IS NOT IN USE NOW IN THIS PAGE 
		function appendDisconnectedSymptom(appendable_materia_medica_symptom_id){
			if(appendable_materia_medica_symptom_id != ""){
				$.ajax({
					type: 'POST',
					url: 'append-newly-added-materia-medica.php',
					data: {
						appendable_materia_medica_symptom_id: appendable_materia_medica_symptom_id
					},
					dataType: "json",
					success: function( response ) {
						console.log(response);
						if(response.status == "success"){
							...
							$.each(resultData, function( key, value ) {
								...
								var uniqueId = value.initial_source_symptom_id+value.comparing_source_symptom_id;
						  		...
						  		// var comparingSymptomHighlightedEndcod = $('<div/>').html(value.comparing_source_symptom_highlighted).text();
						  		var comparingSymptomHighlightedEndcod = (typeof(value.comparing_source_symptom_highlighted) != "undefined" && value.comparing_source_symptom_highlighted !== null && value.comparing_source_symptom_highlighted != "") ? b64DecodeUnicode(value.comparing_source_symptom_highlighted) : "";
					  			...
					  			var displaySymptomString = (typeof(value.initial_source_symptom) != "undefined" && value.initial_source_symptom !== null && value.initial_source_symptom != "") ? b64DecodeUnicode(value.initial_source_symptom) : "";
						  		...
						  		html += '	<th style="width: 14%;" class="text-center">';
						  		html += '		<ul class="info-linkage-group">';
						  		html += '			<li>';
						  		...
						  		html += '			</li>';
						  		html += '			<li>';
						  		html += '				<a title="translation" href="javascript:void(0)" data-item="translate" data-unique-id="'+uniqueId+'"><i class="fas fa-language"></i></a>';
						  		html += '			</li>';
						  		html += '			<li>';
						  		html += '				<a href="javascript:void(0)" id="v_btn_'+uniqueId+'" class="'+vBtnClasses+' '+vBtnDisable+'" title="'+vBtnTitle+'" data-unique-id="'+uniqueId+'" data-v-padding="0" data-initial-symptom-id="'+value.initial_source_symptom_id+'" data-is-connection-loaded="0" data-source-arznei-id="'+value.source_arznei_id+'" data-removable-row-class-chain=""><i class="fas fa-plus"></i></a>';
						  		html += '			</li>';
						  		html += '		</ul>';
						  		html += '	</th>';
						  		if(value.active_symptom_type == "comparing"){
							  		html += '	<th style="width: 14%;" class="">';
							  		html += '		<ul class="command-group">';
							  		html += '			<li>';
		...
		*/
		...
		$(document).on('click', '.nsc', function(){
			...
			if(error_count == 0){
				...
				$.ajax({
					type: 'POST',
					url: 'symptom-connection-operations-view-only.php',
					data: {
