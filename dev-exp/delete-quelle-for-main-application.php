...
<?php  
	...
	try 
	{
		if(isset($_POST['form']) AND !empty($_POST['form'])){	
			...
			if( (isset($formData['quelle_id']) AND !empty($formData['quelle_id'])) AND (isset($formData['arznei_id']) AND !empty($formData['arznei_id'])) AND (isset($formData['id']) AND !empty($formData['id'])) )
			{
				$deleteableQuelleIds = array();
				$deleteableQuelleIds = $formData['id'];
				foreach ($formData['quelle_id'] as $quelleKey => $quelleVal) {
					if (in_array($quelleVal, $deleteableQuelleIds))
  					{
  						$quelle_id = (isset($formData['quelle_id'][$quelleKey]) AND $formData['quelle_id'][$quelleKey] != "") ? mysqli_real_escape_string($db, trim($formData['quelle_id'][$quelleKey])) : null;
						$arznei_id = (isset($formData['arznei_id'][$quelleKey]) AND $formData['arznei_id'][$quelleKey] != "") ? mysqli_real_escape_string($db, trim($formData['arznei_id'][$quelleKey])) : null;
