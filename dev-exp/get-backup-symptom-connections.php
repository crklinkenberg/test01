<?php
	...
	/*
	* Fetching the connections of a particular symptom which are related to the current comparison (This is used in backup section)  
	*/
?>
<?php  
	$resultData = array();
	...
	try {
		if((isset($_POST['initial_symptom_id']) AND $_POST['initial_symptom_id'] != "") AND (isset($_POST['active_symptom_type']) AND $_POST['active_symptom_type'] != "")){	
			...
			$saved_comparisons_backup_id = (isset($_POST['saved_comparisons_backup_id']) AND $_POST['saved_comparisons_backup_id'] != "") ? trim($_POST['saved_comparisons_backup_id']) : null;
			...
			if(mysqli_num_rows($connectedSymptomResult) > 0){
				while($connectedRow = mysqli_fetch_array($connectedSymptomResult)) {
					...
					if(mysqli_num_rows($symptomInfoResult) > 0){
						...
						$FVInfoQuery = mysqli_query($db,"SELECT final_version_de, final_version_en, is_final_version_available FROM final_version_symptoms_info_for_backups WHERE symptom_id = '".$innerWorkingSymptomId."' AND saved_comparisons_backup_id = '".$saved_comparisons_backup_id."'");
						if(mysqli_num_rows($FVInfoQuery) > 0){
							$FVInfoRow = mysqli_fetch_assoc($FVInfoQuery);
							$is_final_version_available = $FVInfoRow['is_final_version_available'];
						} else {
							$is_final_version_available = 0;	
						}
					}
					...
					$ceheckConnectionResult = mysqli_query($db,"SELECT id, is_saved, is_connected, is_pasted, initial_source_id, comparing_source_id FROM symptom_connections_backup WHERE ((initial_source_symptom_id = '".$innerWorkingSymptomId."' OR comparing_source_symptom_id = '".$innerWorkingSymptomId."') AND (initial_source_type = 'original' AND comparing_source_type = 'original')) AND saved_comparisons_backup_id = '".$saved_comparisons_backup_id."' AND (initial_source_symptom_id != '".$innerInitialSymptomId."' AND comparing_source_symptom_id != '".$innerInitialSymptomId."') AND id != '".$connectedRow['id']."' AND (is_connected = 1 OR is_pasted = 1)".$queryForCheckingFurtherConditions);
					...
					if(mysqli_num_rows($originInitialQuelleResult) > 0){
						...
						$FVInfoQuery = mysqli_query($db,"SELECT final_version_de, final_version_en, is_final_version_available FROM final_version_symptoms_info_for_backups WHERE symptom_id = '".$connectedRow['initial_source_symptom_id']."' AND saved_comparisons_backup_id = '".$saved_comparisons_backup_id."'");
						if(mysqli_num_rows($FVInfoQuery) > 0){
							$FVInfoRow = mysqli_fetch_assoc($FVInfoQuery);
							$iniIsFinalVersionAvailable = $FVInfoRow['is_final_version_available'];
						} else {
							$iniIsFinalVersionAvailable = 0;	
						}
					...
					if(mysqli_num_rows($originComparingQuelleResult) > 0){
						...
						$FVInfoQuery = mysqli_query($db,"SELECT final_version_de, final_version_en, is_final_version_available FROM final_version_symptoms_info_for_backups WHERE symptom_id = '".$connectedRow['comparing_source_symptom_id']."' AND saved_comparisons_backup_id = '".$saved_comparisons_backup_id."'");
						if(mysqli_num_rows($FVInfoQuery) > 0){
							$FVInfoRow = mysqli_fetch_assoc($FVInfoQuery);
							$comIsFinalVersionAvailable = $FVInfoRow['is_final_version_available'];
						} else {
							$comIsFinalVersionAvailable = 0;	
						}
			...
			*/
			...
			if($oppositeSymptomId != "")
				$pastedSymptomResult = mysqli_query($db,"SELECT id, saved_comparisons_backup_id, source_arznei_id, initial_source_type, comparing_source_type, is_saved, initial_source_id, comparing_source_id, conversion_initial_source_id, conversion_comparing_source_id, initial_source_code, comparing_source_code, initial_source_symptom_id, comparing_source_symptom_id, initial_source_symptom_highlighted_de, initial_source_symptom_highlighted_en, comparing_source_symptom_highlighted_de, comparing_source_symptom_highlighted_en, initial_source_symptom_de, initial_source_symptom_en, comparing_source_symptom_de, comparing_source_symptom_en, connection_language, matching_percentage, is_connected, is_ns_connect, ns_connect_note, is_pasted, is_ns_paste, ns_paste_note, connection_or_paste_type FROM symptom_connections_backup WHERE ((initial_source_symptom_id = '".$workingSymptomId."' OR comparing_source_symptom_id = '".$workingSymptomId."') AND (initial_source_type = 'original' AND comparing_source_type = 'original')) AND saved_comparisons_backup_id = '".$saved_comparisons_backup_id."' AND (initial_source_symptom_id != '".$oppositeSymptomId."' AND comparing_source_symptom_id != '".$oppositeSymptomId."') AND is_pasted = 1".$queryConditionForComparativeSymptoms." ORDER BY matching_percentage DESC");
			else
				$pastedSymptomResult = mysqli_query($db,"SELECT id, saved_comparisons_backup_id, source_arznei_id, initial_source_type, comparing_source_type, is_saved, initial_source_id, comparing_source_id, conversion_initial_source_id, conversion_comparing_source_id, initial_source_code, comparing_source_code, initial_source_symptom_id, comparing_source_symptom_id, initial_source_symptom_highlighted_de, initial_source_symptom_highlighted_en, comparing_source_symptom_highlighted_de, comparing_source_symptom_highlighted_en, initial_source_symptom_de, initial_source_symptom_en, comparing_source_symptom_de, comparing_source_symptom_en, connection_language, matching_percentage, is_connected, is_ns_connect, ns_connect_note, is_pasted, is_ns_paste, ns_paste_note, connection_or_paste_type FROM symptom_connections_backup WHERE ((initial_source_symptom_id = '".$workingSymptomId."' OR comparing_source_symptom_id = '".$workingSymptomId."') AND (initial_source_type = 'original' AND comparing_source_type = 'original')) AND saved_comparisons_backup_id = '".$saved_comparisons_backup_id."' AND is_pasted = 1".$escapeQueryCondition.$queryCondition." ORDER BY matching_percentage DESC");
			if(mysqli_num_rows($pastedSymptomResult) > 0){
				while($pastedRow = mysqli_fetch_array($pastedSymptomResult)) {
					...
					// $initialOrzId = "";
					// $initialArzId = "";
					$originInitialQuelleResult = mysqli_query($db,"SELECT quelle.jahr, quelle.quelle_type_id, quelle_import_test.original_symptom_id, quelle_import_test.original_quelle_id, quelle_import_test.arznei_id, quelle_import_test.id, quelle_import_test.is_final_version_available FROM quelle_import_test LEFT JOIN quelle ON quelle_import_test.original_quelle_id = quelle.quelle_id WHERE quelle_import_test.id = '".$pastedRow['initial_source_symptom_id']."'");
					if(mysqli_num_rows($originInitialQuelleResult) > 0){
						$originIniQuelleRow = mysqli_fetch_assoc($originInitialQuelleResult);
						$originInitialSourceYear = $originIniQuelleRow['jahr'];

						// $iniIsFinalVersionAvailable = $originIniQuelleRow['is_final_version_available'];
						$FVInfoQuery = mysqli_query($db,"SELECT final_version_de, final_version_en, is_final_version_available FROM final_version_symptoms_info_for_backups WHERE symptom_id = '".$pastedRow['initial_source_symptom_id']."' AND saved_comparisons_backup_id = '".$saved_comparisons_backup_id."'");
						if(mysqli_num_rows($FVInfoQuery) > 0){
							$FVInfoRow = mysqli_fetch_assoc($FVInfoQuery);
							$iniIsFinalVersionAvailable = $FVInfoRow['is_final_version_available'];
						} else {
					...
					if(mysqli_num_rows($originComparingQuelleResult) > 0){
						...
						// $comIsFinalVersionAvailable = $originComQuelleRow['is_final_version_available'];
						$FVInfoQuery = mysqli_query($db,"SELECT final_version_de, final_version_en, is_final_version_available FROM final_version_symptoms_info_for_backups WHERE symptom_id = '".$pastedRow['comparing_source_symptom_id']."' AND saved_comparisons_backup_id = '".$saved_comparisons_backup_id."'");
						if(mysqli_num_rows($FVInfoQuery) > 0){
							$FVInfoRow = mysqli_fetch_assoc($FVInfoQuery);
							$comIsFinalVersionAvailable = $FVInfoRow['is_final_version_available'];
						} else {
