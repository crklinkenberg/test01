...
$(document).on('click', '.connecting-btn', function(){
	...
	{
		var askQuestion = 0;
		if(dataArray['initial_source_year'] != "" && dataArray['comparing_source_year'] != ""){
			if(parseInt(dataArray['comparing_source_year']) <= parseInt(dataArray['initial_source_year'])) 
				askQuestion = 1;
		}

		if(askQuestion == 1) {
			var decision = confirm("Initial source is younger than the comparative one or both sources are from the same year. Do you really want to connect?");
				if (decision) {
					$('#paste_btn_'+uniqueId).prop('disabled', true);
				$('#connecting_btn_'+uniqueId).prop('disabled', true);
				$('#connecting_btn_'+uniqueId).html('<img src="assets/img/loader.gif" alt="Loader">');
...
$(document).on('click', '.paste-btn', function(){
	...
	{
		var con = confirm("Are you sure you want to unpaste?");
			if (con)
			{
				$('#connecting_btn_'+uniqueId).prop('disabled', true);
			$('#paste_btn_'+uniqueId).prop('disabled', true);
			$('#paste_btn_'+uniqueId).html('<img src="assets/img/loader.gif" alt="Loader">');
...
$('body').on( 'submit', '#symptom_search_form', function(e) {
	...
	if(error_count == 0){
		...
		// start the process
		self.search_process_step( 1, 0, data, self );
	}else{
...
function search_process_step( step, number_of_records, data, self ){
			...
			$(document).ready(function () {
			    ...
			    $("#numberOfRecord").html(nummerOfRecordFetch);
			  	if( 'done' == response.step ) {
					...
					self.search_process_step( parseInt( response.step ), nummerOfRecordFetch, data, self );
					// setTimeout(function() {
...
function process_step( step, number_of_records, total_batches_in_part1, total_batches_in_part2, is_stage2_checked, un_matched_symptoms_set_number, data, matched_symptom_ids, self ) {
	...
	$.ajax({
		type: 'POST',
		url: 'get-compare-result-batch-test.php',
		data: {
			form: data,
			step: step,
			...
			else
			{
				if(typeof(response.result_data) != "undefined" && response.result_data !== null) {
					...
					$.each(resultData, function( key, value ) {
				  		...
				  		//console.log(comparingSymptomHighlightedEndcod);
				  		html += '<tr id="row_" class="">';
				  		html += '	<td style="width: 12%;" class="text-center">'+displaySourceCode+'</td>';
				  		html += '	<td>'+displaySymptomString+'</td>';
				  		html += '	<td style="width: 5%;" class="text-center">'+displayPercentage+'</td>';
				  		html += '	<th style="width: 17%;">';
				  		html += '	</th>';
				...
				$(document).ready(function () {
				    // Action after append is completly done
				 	// $('.progress-thead .progress-bar').attr('aria-valuenow', response.progress_percentage);
				 	// $('.progress-thead .progress-bar').animate({
					// 	width: response.progress_percentage + '%',
					// }, 50, function() {
					// 	// Animation complete.
					// 	$('.progress-thead .progress-bar').html(response.progress_percentage+"%");
					// });
