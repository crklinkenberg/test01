	...
	try {
		...
		if(isset($_POST['form']) AND !empty($_POST['form'])){
			...
			if(mysqli_num_rows($totalSymptomQuery) > 0){
				...
				/* if($isExist == 1 AND $is_save_as_new == 1){
					$save_as_new_comparison_name = (isset($_POST['save_as_new_comparison_name']) AND $_POST['save_as_new_comparison_name'] != "") ? mysqli_real_escape_string($db, trim($_POST['save_as_new_comparison_name'])) : null;
					$masterQuery="INSERT INTO saved_comparisons (arznei_id, initial_source_id, comparing_source_ids, comparison_name, similarity_rate, comparison_option) VALUES (NULLIF('".$arzneiId."', ''), '".$initial_source_id."', NULLIF('".$comparing_source_ids."', ''), NULLIF('".$save_as_new_comparison_name."', ''), NULLIF('".$similarity_rate."', ''), NULLIF('".$comparison_option."', ''))";
					$db->query($masterQuery);
					$saved_comparison_master_id = mysqli_insert_id($db);
					if($saved_comparison_master_id != "")
					{
						$result = addQuelleDetailsInSaveOperation($saved_comparison_master_id, $formData);
						if($result['status'] === true)
						{
							...
							$message = "Saved successfully. Closing this message box will reload the comparison data.";
						}
						else
						{
