...
$api->version('v1', ['prefix' => 'v1'], function($api){
	...
	**/
	$api->group(['namespace' => 'App\Http\Controllers\V1', 'middleware' => ['auth:api', 'cors']], function($api){
		...
		//Quelle routes
		$api->get('quelle/all-quelle-zeitschrift', 'QuelleController@allQuelleZeitschrift');
		$api->get('quelle/quelle-settings', 'QuelleController@quelleSettings');
		$api->post('quelle/save-quelle-settings', 'QuelleController@saveQuelleSettings');
		$api->get('quelle/global-grading-settings', 'QuelleController@globalGradingSettings');
		...
		//Global grading sets routes 
		$api->get('global-grading-set/all', 'GlobalGradingController@allGlobalGradeingSets');
		$api->get('global-grading-set/view', 'GlobalGradingController@viewGlobalGradingSet');
		$api->post('global-grading-set/save', 'GlobalGradingController@saveGlobalGradingSet');
		//Synonym De routes
